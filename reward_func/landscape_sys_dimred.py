...
def analyze_results():
    ...
    os.makedirs(f"reward_landscape_grid_search_results/{output_dir}", exist_ok=True)
    
    # Load results
    print(f"Loading results from {args.results_file}...")
    with open(f"reward_landscape_grid_search_results/{args.results_file}", 'rb') as f:
    ...
    plt.savefig(f"reward_landscape_grid_search_results/{output_dir}/{threshold_prefix}reward_umap_visualization.png", dpi=300)
    plt.close()
    ...
    plt.ylabel('UMAP Dimension 2')
    plt.tight_layout()
    plt.savefig(f"reward_landscape_grid_search_results/{output_dir}/{threshold_prefix}reward_heatmap.png", dpi=300)
    plt.close()
    ...
    plt.grid(alpha=0.3)
    plt.tight_layout()
    plt.savefig(f"reward_landscape_grid_search_results/{output_dir}/{threshold_prefix}reward_distribution.png", dpi=300)
    plt.close()
    
    
    
    print(f"Analysis complete! Visualizations saved to reward_landscape_grid_search_results/{output_dir}/ directory")
